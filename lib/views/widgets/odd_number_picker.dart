...
class OddNumberPicker extends StatefulWidget {
  ...
  /// Whether to display only odd numbers
  final bool isOdd;

  const OddNumberPicker({
    Key? key,
  ...
  @override
  _OddNumberPickerState createState() => _OddNumberPickerState();
}

class _OddNumberPickerState extends State<OddNumberPicker> {
  late ScrollController _scrollController;

  @override
  void initState() {
    super.initState();
    final initialOffset = widget.isOdd
        ? ((widget.value - widget.minValue) ~/ (widget.step * 2)) * itemExtent
  ...
  @override
  void didUpdateWidget(OddNumberPicker oldWidget) {
  ...
  int get itemCount {
    if (widget.isOdd) {
      return (widget.maxValue - widget.minValue + 1) ~/ (widget.step * 2) + 1;
    }
    return (widget.maxValue - widget.minValue) ~/ widget.step + 1;
  }
  ...
  int _intValueFromIndex(int index) {
    ...
    index %= itemCount;
    if (widget.isOdd) {
      return widget.minValue + index * widget.step * 2;
    }
